...
class Model(object):
   ...
   def calc_rew(self, xposbefore):
                  ...
                  #new cost for ensuring that vel and accel stay low; chose z vel and accel
                  vel_cost = 0.; accel_cost = 0.
                  for i in range(len(self.ankle_body)):
                  	vel_cost += np.absolute(self.ankle_body[i].GetPos_dt().x)
                  	accel_cost += np.absolute(self.ankle_body[i].GetPos_dtdt().x)
                  #print(vel_cost, accel_cost)
                  print(self.body_abdomen.GetPos().x)
                  # vel and accel tested constant
                  a = 0#.1
