...
ang_vel = np.zeros([num_steps,3])
nu_norm = np.zeros([num_steps,3])
...
#tn = 0
    ...
    while delta > tol and iter < max_iter:
        ...
        if step == 0:
            # Newton order 1
            q_new = q_prev_1 + beta**2*dt**2*q_dd
            qd_new = qd_prev_1 + dt*beta*q_dd 
        else:
            # Newton order 2
        ...
        iter += 1
    pos[step,:] = q_new[:3]
    ang_vel[step,:] = qd_new[:3]
    nu_norm[step,1] = np.linalg.norm(getNu(t))
    step += 1     
t1 = time.time()    
print('finished')  
######################     
...
save2file(pos, "data/Pos.txt")
save2file(ang_vel, "data/Vel.txt")
save2file(nu_norm, "data/Norm.txt")
...
# plotting
plot_from_file("data/Pos.txt", "Translation (m)")
plot_from_file("data/Vel.txt", "Ang_Vel (rad/s)")
plot_from_file("data/Norm.txt", "Norm ()")
